...
/** particle mover */
//called by sputniPIC.cpp at line 96
...
int mover_PC_GPU(struct particles* part, struct EMfield* field, struct grid* grd, struct parameters* param) {
    ...
    struct grid* deviceGrid;
    struct parameters* deviceParam;

    cudaMalloc(&devicePart, sizeof(struct particles));
    cudaMalloc(&deviceField, sizeof(struct EMfield));
    cudaMalloc(&deviceGrid, sizeof(struct grid));
    cudaMalloc(&deviceParam, sizeof(struct parameters));

    // Copy the data to the GPU
    cudaMemcpy(devicePart, part, sizeof(struct particles), cudaMemcpyHostToDevice);
    cudaMemcpy(deviceField, field, sizeof(struct EMfield), cudaMemcpyHostToDevice);
    cudaMemcpy(deviceGrid, grd, sizeof(struct grid), cudaMemcpyHostToDevice);
    cudaMemcpy(deviceParam, param, sizeof(struct parameters), cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(field, deviceField, sizeof(struct EMfield), cudaMemcpyDeviceToHost);
    cudaMemcpy(grd, deviceGrid, sizeof(struct grid), cudaMemcpyDeviceToHost);
    cudaMemcpy(param, deviceParam, sizeof(struct parameters), cudaMemcpyDeviceToHost);
